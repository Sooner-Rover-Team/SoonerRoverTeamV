...
bicepImage = pygame.transform.rotate(pygame.image.load("backgroundPics\ProximalMemberandActuators.png"), -90)
...
initialize = True # Tells the program whether the arm drawing has been initialized yet
bicep_len = 15*scale
...
def draw_arm_stuff(screen, alt_arm_config, claw_x, claw_y):
    ...
    # claw position
    pygame.draw.line(screen, BLACK, (origin_x + claw_x*scale-5, origin_y + claw_y*scale - 5), (origin_x+claw_x*scale+5, origin_y + claw_y*scale+5), width=3)
    pygame.draw.line(screen, BLACK, (origin_x + claw_x*scale-5, origin_y + claw_y*scale+5), (origin_x + claw_x*scale+5, origin_y + claw_y*scale - 5), width = 3)
