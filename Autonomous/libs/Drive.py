...
class Drive:
    ...
    def getSpeeds(self,speed, error, time, kp = .85, ki=.00004):
        values = [0,0]
        ...
        #print("Error Accumulation: ", str(self.errorAccumulation))
    ...
    def printSpeeds(self):
        ...
        print("GPS Coords:" + str(self.gps.latitude) + ", " + str(self.gps.longitude))
    ...
    def trackARMarker(self, id1, id2=-1):
        ...
        while self.tracker.angleToMarker > 14 or self.tracker.angleToMarker < -14:
            if self.tracker.findMarker(id1, id2, cameras=1): #Only looking with the center camera right now
                ...
                timesNotFound = 0
            elif timesNotFound == -1:
                print("STOP USING ME IM PIVOTING PLEASE")
                sleep(2)#Never seen the tag with the main camera
                if(math.ceil(int(count/20)/5) % 2 == 1):
                    self.speeds = [self.baseSpeed+2,-self.baseSpeed-2]
                else:
                    self.speeds = [-self.baseSpeed-2,self.baseSpeed+2]
            elif timesNotFound < 5: #Lost the tag for less than 1.5 seconds after seeing it with the main camera
                timesNotFound += 1
