...
class ARTracker:
    ...
    def arFound(self, id, image, writeToFile):
        ...
        cv2.cvtColor(image, image, cv2.COLOR_RGB2GRAY)  
        ...
        for i in range(40, 221, 60):
            ...
            #aruco.detectMarkers((image > i), self.tagDict, self.corners, self.markerIDs, self.rejects)
            (self.corners, self.markerIDs, self.rejected) = aruco.detectMarkers(image, self.tagDict)   
        ...
        widthOfTag = self.corners[index][1].x - self.corners[index][0].x 
        distanceToAR = (self.knownTagWidth * self.focalLength) / widthOfTag 
    
        centerXTag = (self.corners[index][1].x + self.corners[index][0].x) / 2 
