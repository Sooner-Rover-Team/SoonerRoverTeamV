...
CONT_CONFIG = config["Controller"][
    "CONFIG"
]  # change this in config if using a different controller than the old xbox 360
...
""" Socket stuff """
ebox_socket = socket.socket(
    socket.AF_INET, socket.SOCK_DGRAM
)  # a remote socket where the IP/port are the ones on the microcontroller
if USING_BRIDGE:
    ebox_socket.connect(
        (BRIDGE_HOST, EBOX_PORT)
    )  # What is the bridge? The base station/rover antenna connection?
else:
    ebox_socket.connect(
        (EBOX_HOST, EBOX_PORT)
    )  # if not using base station, make the microcontroller the host? why not laptop?
...
# old variables are used to more efficiently send UDP by not sending identical messages
# over and over again which is pointless.
old_act_speed = 0
old_carousel_turn = 0
old_claw_position = 0
...
def wheel_message(leftwheels, rightwheels):
    msg = bytearray(
        [
            0x01,
            0x01,  # ID for WHEEL system and secondary ID for actual wheels
            leftwheels[0],
            leftwheels[1],
            leftwheels[2],
            rightwheels[0],
...
def arm_messge(
    shoulder_length, elbow_length, base_rotation, wrist_angle, wrist_rotation, claw_dir
):
    msg = bytearray(8)
...
def fix(number):
    return floor(number) if number % 1 <= 0.5 else ceil(number)
...
if __name__ == "__main__":
    ...
    while running:
            ...
            if event.type == pygame.JOYBUTTONDOWN:
                ...
                if mode == "drive":
                    ...
                    if event.button == X_BUTTON:  # X button switches modes
                        ...
                        ebox_socket.sendall(msg)
                    if (
                        event.button == Y_BUTTON
                    ):  # Y button toggles sport mode (max speed)
        ...
        if (
            joystick.get_button(SELECT)
            and joystick.get_button(START)
            and mode == "drive"
        ):
            running = False
        ...
        gim = (
            0  # joystick.get_hat(gimbal) # size 2, gim[0]= left/right, gim[1]= up/down
        )
        ...
        if mode == "drive":
            ...
            if sportMode:
                leftwheels = [
                    int(val_map(wheel, 0, 252, 63, 189)) for wheel in leftwheels
                ]
                rightwheels = [
                    int(val_map(wheel, 0, 252, 63, 189)) for wheel in rightwheels
                ]
            ...
            )  # gim+1 to avoid sending negative #s
        ...
        # send message to move the arm
            ...
            shoulder_length, elbow_length, temp_u, temp_v = util.arm_calc(
                alt_arm_config, temp_u, temp_v
            )
            ...
            if abs(R_Y) > THRESHOLD_HIGH:
                wrist_angle = 126 + int(
                    R_Y * 42
                )  # for fine mainupulation change this to 32 (currently 55)
            else:
                wrist_angle = 126
        ...
        # A button toggles claw open/close (might change to right joystick)
            ...
            if (
                abs(L_Y) > THRESHOLD_HIGH
            ):  # left stick vertical axis controls linear actuator -1 to +1. Send 0 to 255. 0-127=backward speed. 127=stop, 127-255=forward speed
            ...
            if (
                messageIsDifferent(
                    [act_speed, carousel_turn, claw_position, microscope_position],
                    lastScienceMsg,
                )
                or numSameMessages > 5
            ):
        ...
        if mode == "drive":
            ...
            util.draw_science_stuff(
                screen,
                (act_speed, microscope_position, claw_position, carousel_turn),
                tp,
            )
        ...
        timer.tick(
            40
        )  # FPS 40 - Test once subsystems are built to see how fast we can send messages without big errors or stalls in the arduino function
